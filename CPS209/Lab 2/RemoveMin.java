...
public class RemoveMin
{
    ...
    public static void removeMin(ArrayList<Integer> integerList) {
        
        if(integerList.size() > 0){
            int smallestNum = integerList.get(0);
            int indexNum = 0;

            for(int pos = 0; pos < integerList.size(); pos++){
                if(integerList.get(pos) < smallestNum){
                    indexNum = pos;
                    smallestNum = integerList.get(pos);
                    }
            }
        integerList.remove(indexNum);
        }
