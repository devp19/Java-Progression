...
public class PointTester
{
    ...
    {
        //-----------Start below here. To do: approximat e lines of code = 7
        //
        ...
        for(int pos = 0; pos < points.size(); pos++){
            ...
            for(int innerpos = 0; innerpos < points.size(); innerpos++){

                double comparator = getDistance(points.get(pos), points.get(innerpos));
                
                if(comparator > maxDistance){
                    maxDistance = comparator;
    ...
    public static ArrayList<Point> getUniquePoints(ArrayList<Point> points)
    {
        ...
        for (int i = 0; i < points.size(); i++){
            if(original.contains(points.get(i))){
                continue; // next point 
            }
            else{
                original.add(points.get(i));
            }
        }
        return(original);
